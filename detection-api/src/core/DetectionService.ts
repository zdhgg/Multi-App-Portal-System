...
import {
  ...
  type AggregatedProject as PipelineAggregatedProject
} from './pipeline'

// 特性开关：是否使用新管道架构
const USE_PIPELINE = process.env.USE_DETECTION_PIPELINE !== 'false'
...
export class DetectionService implements IDetectionService {
   ...
   * 
   * 支持两种模式：
   * 1. 新管道架构（USE_PIPELINE=true，默认）
   * 2. 旧聚合器架构（USE_PIPELINE=false）
   */
  async getEnhancedAggregatedProjects(sessionId: string): Promise<readonly EnhancedAggregatedProject[]> {
    // 使用新管道架构
    if (USE_PIPELINE) {
      return this.getEnhancedAggregatedProjectsViaPipeline(sessionId)
    }

    // 使用旧聚合器架构（保留作为回退）
    return this.getEnhancedAggregatedProjectsLegacy(sessionId)
  }

  /**
   * 新管道架构实现
   */
  private async getEnhancedAggregatedProjectsViaPipeline(sessionId: string): Promise<readonly EnhancedAggregatedProject[]> {
    logger.info('[Pipeline] Getting enhanced aggregated projects via pipeline', { sessionId })
    ...
    try {
      ...
      const existingApps = await this.getExistingAppsMap()

      // 构建扫描配置，包含端口配置
      const scanConfig = createScanConfigFromSession(session.config)

      // 注入端口配置
      if (portConfig) {
        scanConfig.portAllocation.frontendRange = portConfig.frontendRange || { start: 3001, end: 3100 }
        scanConfig.portAllocation.backendRange = portConfig.backendRange || { start: 8001, end: 8100 }
        scanConfig.portAllocation.reservedPorts = portConfig.reservedPorts?.map((p: any) => p.port) || []
      }

      // 注入已分配的端口（从已添加的应用中获取）
      const allocatedPorts: number[] = []
      for (const app of existingApps.values()) {
        if (app.network?.primaryPort) allocatedPorts.push(app.network.primaryPort)
        if (app.network?.secondaryPorts) allocatedPorts.push(...app.network.secondaryPorts)
      }
      scanConfig.portAllocation.allocatedPorts = allocatedPorts
      ...
      logger.info('[Pipeline] Enhanced project aggregation completed', {
        sessionId,
        success: result.success,
        projectCount: result.projects.length,
        fullStackCount: result.projects.filter(p => p.type === 'fullstack').length,
        duration: `${duration}ms`,
        stageTimings: result.context.metadata.stageTimings
      })

      // 转换为 EnhancedAggregatedProject 格式（使用已获取的 existingApps）
      return await this.convertPipelineProjectsToEnhanced(result.projects, existingApps)

    } catch (error) {
      logger.error('[Pipeline] Enhanced project aggregation failed, falling back to legacy', { sessionId, error })
      // 降级到旧架构
      return this.getEnhancedAggregatedProjectsLegacy(sessionId)
    }
  }
   ...
   * 获取已添加应用的映射表（目录路径 -> 应用）
   */
  private async getExistingAppsMap(): Promise<Map<string, Application>> {
    ...
    try {
      ...
      for (const app of apps) {
        appsMap.set(this.normalizeDirectory(app.directory), app)
      }
    } catch (error) {
      logger.warn('[Pipeline] Failed to get existing apps', { error })
    }
   ...
   * 旧聚合器架构实现（保留作为回退）
   */
  private async getEnhancedAggregatedProjectsLegacy(sessionId: string): Promise<readonly EnhancedAggregatedProject[]> {
    logger.info('[Legacy] Getting enhanced aggregated projects', { sessionId })
    ...
    try {
      ...
      logger.info('[Legacy] Enhanced project aggregation completed', {
        sessionId,
        originalCount: rawResults.length,
      ...
      logger.error('[Legacy] Enhanced project aggregation failed', { sessionId, error })
      // 降级到传统聚合
  ...
  /**
   * 将管道项目转换为 EnhancedAggregatedProject 格式
   * 如果项目已添加到应用管理，使用已分配的端口
   */
  private async convertPipelineProjectsToEnhanced(
    projects: PipelineAggregatedProject[],
    existingApps: Map<string, Application>
  ): Promise<EnhancedAggregatedProject[]> {
    return projects.map(p => {
      // 查找已添加的应用
      const existingApp = existingApps.get(this.normalizeDirectory(p.directory))

      // 如果已添加，使用已分配的端口
      let frontendPort = p.ports.frontend
      let backendPort = p.ports.backend
      let mainPort = p.ports.main

      if (existingApp) {
        mainPort = existingApp.network?.primaryPort
        // 如果是全栈项目，检查是否有 secondaryPorts
        if (existingApp.network?.secondaryPorts?.length) {
          frontendPort = existingApp.network.primaryPort
          backendPort = existingApp.network.secondaryPorts[0]
        }
      }
      ...
      return {
        ...
        type: p.type === 'unknown' ? 'frontend' : p.type,
        components: {
          frontend: p.fullStackInfo?.frontend ? {
            id: p.fullStackInfo.frontend.id,
            sessionId: '',
            directory: p.fullStackInfo.frontend.directory,
            techStack: p.fullStackInfo.frontend.techStack,
            confidence: p.fullStackInfo.frontend.confidence,
            issues: [],
            createdAt: Date.now(),
            originalPort: frontendPort
          } : undefined,
          backend: p.fullStackInfo?.backend ? {
            id: p.fullStackInfo.backend.id,
            sessionId: '',
            directory: p.fullStackInfo.backend.directory,
            techStack: p.fullStackInfo.backend.techStack,
            confidence: p.fullStackInfo.backend.confidence,
            issues: [],
            createdAt: Date.now(),
            originalPort: backendPort
          } : undefined
        },
        fullstackInfo: p.fullStackInfo ? {
          detectionType: p.fullStackInfo.type,
          frontendPath: p.fullStackInfo.frontend.directory,
          backendPath: p.fullStackInfo.backend.directory,
          portAllocation: {
            strategy: 'separated',
            ports: {
              frontend: frontendPort || 3001,
              backend: backendPort || 8001
            },
            reasoning: existingApp ? ['使用已分配端口'] : ['管道架构自动分配']
          },
          originalPorts: {
            frontend: [],
            backend: []
        ...
        primaryTechStack: p.techStacks[0] || 'unknown',
        techStacks: p.techStacks,
        confidence: p.confidence,
        componentCount: p.applications.length,
        isBackup: p.isBackup,
        description: p.description,
        detectionReason: existingApp ? '已添加应用' : '管道架构检测'
      }
    })
  }
  ...
  async detectSingleDirectoryEnhanced(directory: string, options?: any): Promise<{
    ...
    try {
      ...
      if (fullStackProjects.length > 0) {
        ...
        // 🔧 修复：对前端和后端子目录分别独立检测，不再复用同一个 traditional 对象
        let frontendComponent: DetectionResult & { originalPort?: number } | undefined
        let backendComponent: DetectionResult & { originalPort?: number } | undefined

        try {
          const frontendResult = await this.detectSingleDirectory(fullStackProject.frontend.directory, options)
          frontendComponent = {
            ...frontendResult,
            originalPort: fullStackProject.frontend.ports[0]
          }
        } catch (err) {
          logger.warn('前端子目录检测失败，跳过 components.frontend', {
            directory: fullStackProject.frontend.directory,
            error: err
          })
        }

        try {
          const backendResult = await this.detectSingleDirectory(fullStackProject.backend.directory, options)
          backendComponent = {
            ...backendResult,
            originalPort: fullStackProject.backend.ports[0]
          }
        } catch (err) {
          logger.warn('后端子目录检测失败，跳过 components.backend', {
            directory: fullStackProject.backend.directory,
            error: err
          })
        }
...
interface FileScanner {
  findApplicationDirectories(workspacePath: string, options?: { maxDepth?: number }): Promise<string[]>
}
