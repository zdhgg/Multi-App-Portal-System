...
- **统一端口清单**: 监听列表、容量统计、归属核验和冲突提示共享同一份系统快照
- **端口操作安全增强**: 强制释放重新核验 PID，失效分配支持管理员预览确认，系统与门户保留端口禁止直接释放
- **全栈启动就绪检查**: 前后端子进程必须真正监听端口才会确认启动成功，失败时自动清理部分启动进程
...
- 🔌 **统一端口清单**
  - 新增统一端口清单服务，容量统计、监听列表、归属核验和冲突信息由同一份系统快照生成
  - 区分已核验、PID 不匹配、重复配置、未托管、无法核验和保留端口，并展示真实进程与应用归属
- 🛡️ **安全操作流程**
  - 强制释放会重新核验当前 PID，拒绝释放系统保留端口、已托管应用端口和已经变化的监听进程
  - 失效分配改为管理员先预览、再确认清理，避免无提示删除端口分配记录
- 🔄 **监控与状态一致性**
  - 端口页面展示数据新鲜度和实时连接状态；刷新失败保留最后成功快照，不再展示伪造容量
  - 并发刷新只允许最新请求更新界面，WebSocket 同时兼容新 `payload` 和旧 `data` 消息
- 🚀 **应用启动可靠性**
  - 全栈启动等待前后端端口真正就绪，失败时清理部分启动进程，并为冷启动提供更合理的宽限期
  - 启动完成或端口冲突后重新核验运行态，避免旧离线快照覆盖已经成功启动的应用
...
- 🧩 **system-config 解析链路统一**
  ...
  - 避免 `system-config.json` 被 Windows 编辑器保存为 UTF-8 BOM 后出现 `JSON.parse` 失败或配置读取不完整
- 💾 **自动备份调度兼容性修复**
  - 自动备份调度器现在可以稳定读取带 BOM 的系统配置，不再因为首字符异常导致计划未启动
  - 补充对应回归测试，覆盖 BOM 文件下的备份时间、保留天数和输出路径读取
- 🔢 **发布资料与版本对齐**
  - 同步根项目、前后端包、系统配置、启动脚本、前端内置更新记录和主要发布文档到 `1.3.1`
