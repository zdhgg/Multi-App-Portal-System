...
// 获取应用列表
GET /api/apps
Query: { page?, limit?, status?, techStack?, search? }
...
// 创建应用
POST /api/apps
Body: { name, description?, directory, techStack, ... }

// 从检测结果创建应用
POST /api/apps/from-detection
Body: { scanId, detectionId, name?, description? }
...
// 批量更新应用状态
PATCH /api/apps/batch/status
Body: { ids: string[], status: string }

// 获取智能启动命令
POST /api/apps/:id/start-command
Body: { mode?: 'development' | 'production', port?: number }
Response: { command: string, args: string[] }
```
...
Response: {
  ...
  args: string[],
  techStack: string,
  portInjected: boolean
}

// 验证技术栈支持
GET /api/command/tech-stacks
Response: {
  supported: Array<{
    name: string,
    variants: string[],
    supportsPortInjection: boolean,
...
Body: { directory: string, techStack: string, port: number }
Response: {
  success: boolean,
...
Body: { workspacePath, config? }

// 获取扫描状态
GET /api/detection/scan/:scanId

// 获取扫描结果
GET /api/detection/results/:scanId

// 取消扫描
POST /api/detection/scan/:scanId/cancel
...
Body: { appId, types?, domain?, outputFormat? }

// 批量生成配置
POST /api/config/batch-generate
Body: { appIds: string[], types?, domain? }

// 预览配置
POST /api/config/preview
Body: { appId, type, templateName? }

// 应用配置到目录
POST /api/config/apply
Body: { appId, configs: Array<{filename, content, type}>, overwrite? }
```
...
- [x] 精确扫描深度控制
- [x] 智能端口分配优化
- [x] RBAC 角色权限控制（admin/operator/guest）
- [x] JWT 认证硬化 + Token 黑名单
- [x] 会话管理 + 审计日志
- [x] 文件系统硬化 + CORS 收敛

### 短期计划 (v2.1)
- [ ] 支持更多技术栈（Spring Boot、Django、Flask 等）
- [ ] 添加应用性能监控
- [ ] 实现应用依赖关系图
- [ ] 增强配置模板系统
