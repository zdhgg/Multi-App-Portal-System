    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.startsWith('.') || entry === 'node_modules') {
          logger.debug('Skipping hidden or node_modules directory', { entry })
        ...
        if (stats.isDirectory()) {
          ...
          await this.scanDirectory(fullPath, results, currentDepth + 1, maxDepth)
        }
      }
    } catch (error) {
...
`
   ...
   * 获取进程日志
   */
  getProcessLogs(appId: string, lines?: number): string[] {
    const processInfo = this.processes.get(appId)
    if (!processInfo) {
      return []
    }
